...
# Baseconnect分類のAIファクトチェック
INDUSTRY_FACTCHECK_AI_ENABLED = os.getenv("INDUSTRY_FACTCHECK_AI_ENABLED", "true").lower() == "true"
INDUSTRY_FACTCHECK_MIN_CONFIDENCE = float(os.getenv("INDUSTRY_FACTCHECK_MIN_CONFIDENCE", "0.75"))
        ...
        while True:
            ...
            try:
                try:
                    ...
                    capital_val = clean_amount_value(company.get("capital") or "")
                    employees_val = clean_employee_value(company.get("employees") or "")
                    industry_top = (company.get("industry_top") or "").strip()
                    industry_sub = (company.get("industry_sub") or "").strip()
                    ...
                    company.setdefault("industry_top", industry_top)
                    company.setdefault("industry_sub", industry_sub)
                    ...
                    if (
                        ...
                        try:
                            ...
                            generated = await asyncio.wait_for(
                                verifier.generate_description(desc_text, bytes(shot), name, addr_raw, industry_hint=""),
                    ...
                    if fatal_error:
                        ...
                        # ---- Baseconnect category classification (final) ----
                        ...
                        profile_blocks = _collect_profile_text_blocks_all(profile_payloads)

                        extra_blocks: list[str] = []
                        if not profile_blocks:
                            extra_blocks = _collect_business_text_blocks(payloads_for_desc)
                            if name:
                                extra_blocks.append(name)

                        classify_result = INDUSTRY_CLASSIFIER.classify(
                            description=description_val or '',
                            business_tags=parsed_business_tags,
                            profile_blocks=profile_blocks,
                            extra_blocks=extra_blocks,
                            company_name=name,
                        )
                        industry_top = (classify_result.get('top') or industry_top or '').strip()
                        industry_sub = (classify_result.get('sub') or industry_sub or '').strip()
                        industry_val = industry_sub or industry_top

                        # ---- AI fact-check for Baseconnect classification ----
                        if (
                            INDUSTRY_FACTCHECK_AI_ENABLED
                            and USE_AI
                            and verifier is not None
                            and hasattr(verifier, "verify_baseconnect_industry")
                            and (industry_top or industry_sub)
                        ):
                            candidates = INDUSTRY_CLASSIFIER.rank_candidates(
                                description=description_val or "",
                                business_tags=parsed_business_tags,
                                profile_blocks=profile_blocks,
                                extra_blocks=extra_blocks,
                            ...
                            if industry_top:
                                exists = any(
                                    (c.get("top") == industry_top and (c.get("sub") or "") == industry_sub)
                                    for c in candidates
                                    if isinstance(c, dict)
                                )
                                if not exists:
                                    candidates.insert(0, {"top": industry_top, "sub": industry_sub, "score": 0})

                            lines: list[str] = []
                            for idx, cand in enumerate(candidates[:10]):
                                top = str(cand.get("top") or "").strip()
                                sub = str(cand.get("sub") or "").strip()
                                if not top:
                                    continue
                            ...
                            try:
                                ai_res = await asyncio.wait_for(
                                    verifier.verify_baseconnect_industry(
                                        text=ai_text,
                                        company_name=name,
                                        rule_top=industry_top,
                                        rule_sub=industry_sub,
                                        candidates_text=candidates_text,
                                        max_snippet_chars=INDUSTRY_AI_MAX_SNIPPET_CHARS,
                                    ),
                                    timeout=min(clamp_timeout(ai_call_timeout), 25.0),
                                )
                            except Exception:
                                ai_res = None
                            ...
                            if isinstance(ai_res, dict):
                                try:
                                    ...
                                    conf = 0.0
                                top_ok = ai_res.get("top_ok") is True
                                sub_ok = ai_res.get("sub_ok") is True
                                if conf < INDUSTRY_FACTCHECK_MIN_CONFIDENCE:
                                    top_ok = False
                                    sub_ok = False
                                if not top_ok:
                                    industry_top = ""
                        ...
                        company['industry_top'] = industry_top
                        company['industry_sub'] = industry_sub
                        company['business_tags'] = business_tags_val

                        final_industry_for_desc = industry_val or ''
                        description_val = _ensure_name_industry_in_description(
                            description_val,
                            name,
