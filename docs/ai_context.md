...
5) 公式サイトから候補抽出（電話/住所/代表者/財務/説明）。足りなければ優先リンクを最大10件以上巡回し、必要に応じて関連クロールを広げる。`TIME_LIMIT_SEC` 超過時は深掘り・2回目 AI をスキップ。
...
- 説明: 「何をしている会社か」「提供サービス/事業内容」がわかる本文を80文字以内で要約。見出しだけ（会社概要/事業概要/法人概要/沿革/会社案内/企業情報）は除外。ニュース/採用/日付行は除外。事業・製造・開発・販売・提供・サービス・運営・支援・施工・設計・製作などの事業キーワードを含まない文は除外。
