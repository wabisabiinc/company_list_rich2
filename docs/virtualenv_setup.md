...
- `REP_REQUIRE_STRUCTURED_SOURCE`（デフォルト `true`）は、代表者名を構造化ソース（TABLE/LABEL/ROLE/JSONLD）由来のみに限定し、誤格納（UI文言など）を構造的に防ぎます（代表者が取れないケースは増えます）
- description を毎回AIで作りたい場合は `AI_DESCRIPTION_ALWAYS=true`（既定true）と `AI_DESCRIPTION_ALWAYS_CALL=true`（既定true）を使います（AI生成に失敗した場合は既存値を保持します）
- 優先docs（会社概要等）の取得リンク数は `PRIORITY_DOCS_MAX_LINKS_CAP` / `PROFILE_DISCOVERY_MAX_LINKS_CAP` で上限を下げられます（時間爆発防止）
