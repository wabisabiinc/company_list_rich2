...
class AIVerifier:
    ...
    def _build_prompt(self, text: str, company_name: str = "", address: str = "") -> str:
        ...
        prompt = (
            ...
            "- description には業種を必ず含める（本文で根拠が弱い場合は description を null にする）。\n"
            "厳守:\n"
    ...
    def _build_description_prompt(self, text: str, company_name: str = "", address: str = "", industry_hint: str = "") -> str:
        ...
        prompt = (
            ...
            "- 業種を必ず含める（業種ヒントがある場合はそれを優先。根拠が弱い場合は null）\n"
    ...
    def _build_rich_prompt(self, payload_json: str, company_name: str = "", csv_address: str = "") -> str:
        ...
        prompt = (
            ...
            "- description には会社名を必ず含める（対象企業の表記をそのまま使用）。\n"
            "- description には業種を必ず含める（根拠が弱い場合は null）。\n"
    ...
    async def judge_official_homepage(
        ...
        prompt = (
            ...
            "- description を返す場合は会社名と業種を必ず含める（業種根拠が弱い場合は null）。\n"
